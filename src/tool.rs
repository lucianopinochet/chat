...
pub async fn coroutine_handle(mut rx: UnboundedReceiver<String>, messages:Arc<Mutex<&UseRef<Vec<String>>>>){
    ...
    loop{
      tokio::select! {
        result = reader.read(&mut buffer) => {
          messages.lock().await.write().push(String::from_utf8_lossy(&buffer).to_string());
          buffer = [0;64];
          if let Err(_) = result{
